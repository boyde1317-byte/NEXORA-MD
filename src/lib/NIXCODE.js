...
// ─── NEXORA-MD adaptation (vendored 2026-09-13) ──────────────────────────────
// Vendored verbatim from Moonson lib/NIXCODE.js v4.5 (Nixel, header above).
// Only two functional differences, both documented here:
//   1. `sharp` is imported lazily inside AIRich.resize() — NEXORA does not
//      install sharp and resize() is not used by the rich-response path.
//   2. AIRich.send() runs the built content through
//      generateWAMessageFromContent before relayMessage (raw baileys
//      relayMessage expects a generated WAMessage; Moonson's socket wrapper
//      did this internally).
// Everything else — including the AIRich GenAI rich-response builder used by
// Moonson's .ping (addProduct/addText/addTip/addSuggest) and .facebookdl
// (addVideo/addText/addTip) — is untouched upstream code.
// ─────────────────────────────────────────────────────────────────────────────
...
class ButtonV2 extends BaseBuilder {
	...
	async build(jid, { ...options } = {}) {
		// NEXORA patch: a buttonsMessage location header without a jpegThumbnail
		// renders blank. Default to the same brand image the menu uses (unless
		// .image() was called). Any fetch/resize failure falls back to the
		// pre-patch behavior (no thumbnail) — a failed header image must never
		// break the card send. Attribution to itsliaaa unchanged.
		...
		if (_thumbSrc) {
			try {
				const _buf = Buffer.isBuffer(_thumbSrc) ? _thumbSrc : await BaseBuilder.fetchBuffer(_thumbSrc, {}, { silent: true });
				if (_buf && _buf.length) _thumbnail = await BaseBuilder.resize(_buf, 300, 300);
			} catch (_) {
				_thumbnail = null;
			}
		}
...
class AIRich extends BaseBuilder {
	...
	async send(jid, { forwarded, includesUnifiedResponse, includesSubmessages, ...options } = {}) {
		...
		const message = await generateWAMessageFromContent(jid, msg, {
			userJid: this.#client.user?.id || '0@s.whatsapp.net',
		});

		await this.#client.relayMessage(jid, message.message, { messageId: message.key.id });
		return message;
	}
