...
// Replace from 'async sendRichResponse' to 'async sendCarousel' (excluding sendCarousel)
const startIdx = content.indexOf('async sendRichResponse');
const endIdx = content.indexOf('async sendCarousel');
if (startIdx !== -1 && endIdx !== -1) {
  content = content.substring(0, startIdx) + newMethod + '\n\n  ' + content.substring(endIdx);
  fs.writeFileSync(file, content);
  console.log('Fixed baileysBridge.js');
} else {
